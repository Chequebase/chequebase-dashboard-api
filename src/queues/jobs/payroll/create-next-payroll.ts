import PayrollSetting, { IPayrollSetting } from "@/models/payroll/payroll-settings.model";
...
async function createNextPayrolls(_: Job) {
  ...
  logger.log("creating organization payrolls", { month: today.format('MMMM, YYYY') });
...
async function createPayroll(
  ...
  try {
    ...
    const wallet = await Wallet.findOne({ organization: setting.organization, type: WalletType.Payroll })
    if (!wallet) {
      logger.error('could not find wallet', { organization: setting.organization, })
      throw new Error('unable to find wallet for organization ' + setting.organization)
    }
    ...
    logger.error("error occured creating payroll", { reason: err.message, stack: err.stack });
  }
}
