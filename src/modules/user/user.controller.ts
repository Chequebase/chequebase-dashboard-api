...
export default class UserController {
  ...
  async reactivate(@Req() req: Request, @Res() res: Response) {
    try {
      ...
      if (result.success) {
        return res.redirect(
          `${getEnvOrThrow("BASE_FRONTEND_URL")}/auth/signin`
        );
      }
      ...
      logger.error("error during user reactivation", {
        reason: err.message,
        stack: err.stack,
      });
  ...
  registerIndividual(@Body() registerIndividualDto: RegisterIndividualDto) {
    return this.userService.registerIndividual(registerIndividualDto);
  }
  ...
  uploadAvatar(@CurrentUser() auth: AuthUser, @Req() req: Request) {
  ...
  getMembers(
    @CurrentUser() auth: AuthUser,
    @QueryParams() query: GetMembersQueryDto
  ) {
